...
const ImportDialog = () => {
  const [selectedTemplate, setSelectedTemplate] =
    React.useState<TemplateFile>(null)
  const [selectedFile, setSelectedFile] = React.useState<importedFile>(null)
  ...
  const handleImportFiles = () => {
    const allGood = importedFiles.map((f) => {
      return handleImport({
        ...
        selected: { col: 0, row: selectedTemplate.template.row },
      })
    })
  ...
  return (
    <Dialog open={open}>
      <DialogTrigger onClick={() => setOpen((prev) => !prev)}>
        <CustomTooltip
          title='Import files'
          Icon={<LucideImport className='h-5 w-5' />}
        />
      </DialogTrigger>
