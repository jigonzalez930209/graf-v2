...
const DragDrop = ({ PlotlyChart }: { PlotlyChart: JSX.Element }) => {
  ...
  const handleApply = () => {
    ...
    if (itemGroups.yAxis.length < itemGroups.y2Axis.length) {
      console.warn(
        'Y2 axis should be have maximum the number of column of Y1 Axis'
      )
      enqueueSnackbar(
        'Y2 axis should be have maximum the number of column of Y1 Axis',
        { variant: 'error' }
    ...
    if (
      itemGroups.xAxis.length > 1 &&
      itemGroups.xAxis.length !== itemGroups.yAxis.length
    ) {
    ...
    if (
      itemGroups.xAxis.length > 1 &&
      itemGroups.xAxis.length < itemGroups.y2Axis.length
    ) {
      console.warn('Y2 should be at less the same number of column of X axis')
      enqueueSnackbar(
        'Y2 should be at less the same number of column of X axis',
        { variant: 'error' }
      )
      return
    }
    ...
    if (!_.isEmpty(fileColumns)) {
      setTimeout(() => {
        setItemGroups({
          ...
          xAxis:
            fileColumns.x?.map((d) => ({ name: d.name, index: d.index })) || [],
          yAxis:
            fileColumns.y?.map((d) => ({ name: d.name, index: d.index })) || [],
          y2Axis:
            fileColumns.y2?.map((d) => ({ name: d.name, index: d.index })) ||
            [],
        })
      }, 1)
    } else {
